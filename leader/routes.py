from flask import Blueprint, render_template, session, jsonify, request, redirect, url_for, make_response, current_app
...
def generate_pdf_report(project, stats, team_data, tasks_by_status=None, tasks_by_priority=None, recent_activities=None, completion_rate=0):
    ...
    if tasks_by_status:
        ...
        story.append(Spacer(1, 10))
        status_data = [['Status', 'Count']] + [[s[0], str(s[1])] for s in tasks_by_status]
    ...
    if tasks_by_priority:
        ...
        story.append(Spacer(1, 10))
        priority_data = [['Priority', 'Count']] + [[p[0], str(p[1])] for p in tasks_by_priority]
    ...
    if recent_activities:
        ...
        for activity in recent_activities:
            desc = f"Completed task: {activity[1]}" if activity[0] == 'task_completed' else f"Joined as {activity[1]}"
...
def my_team_page():
    ...
    avg_productivity = round(completed_tasks / total_members, 1) if total_members > 0 else 0
    ...
    on_time_rate = round((delivery_stats[1] / delivery_stats[0] * 100), 1) if delivery_stats[0] > 0 else 0
...
def my_profile():
    ...
    project_list = [
        {
            'id': p[0], 'name': p[1],
            'status': p[2] or 'active', 'progress': p[3] or 0,
...
def dashboard():
    ...
    recent_activities = [
        {'type': a[0], 'description': a[1], 'user_name': a[2], 'created_at': a[3], 'project_name': a[4]}
        for a in activities
    ]
...
def export_pdf():
    ...
    buffer = generate_pdf_report(project, stats, team_data, tasks_by_status, tasks_by_priority, completion_rate=completion_rate)
...
def email_report():
    ...
    buffer = generate_pdf_report(
        project, stats, team_data,
        tasks_by_status, tasks_by_priority,
        recent_activities, completion_rate
    )
...
def get_task(task_id):
    ...
    return jsonify({
        'task_id': task[0], 'title': task[1], 'description': task[2],
        'assigned_to': task[3], 'project_id': task[4], 'priority': task[5],
        'status': task[6], 'due_date': task[7].strftime('%Y-%m-%d') if task[7] else None
    })
...
def update_profile():
    ...
    try:
        cur.execute("""
            ...
            WHERE user_id = %s
            RETURNING user_id;
        """, (request.form.get("name"), request.form.get("email"), request.form.get("designation"), leader_id))
...
def change_password():
    ...
    try:
        ...
        cur.execute("""
            ...
            WHERE user_id = %s;
        """, (new_password, leader_id))  # NOTE: Hash the password before storing!
